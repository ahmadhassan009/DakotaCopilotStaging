...
export default class MultiSelectComboboxCustom extends LightningElement {
  @api disabled = false;
  @api label = '';
  @api name;
  @api options = [];
  ...
  @api handleListChange(updatedList, anySelectedValue) {
    this.options=updatedList;
    if(!anySelectedValue) {
  ...
  handleClick() {
    ...
    if (this.template.querySelector('.slds-is-open')) {
      ...
      this.dispatchEvent(new CustomEvent('storeoldvalues', { detail: this.getSelectedItems() }));
    }
  }
  change(event) {
